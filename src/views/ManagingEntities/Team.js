...
const Team = () => {
    ...
    const updateData = useCallback(async () => {
        ...
        let response1 = await teamService.findTeam(teamId);
    ...
    const addToTeam = (team_id) => async (user_id) => {
        await teamService.addUserToTeam(team_id, user_id);
        updateData();
    };

    const removeFromTeam = (team_id) => async (user_id) => {
        await teamService.removeFromTeam(team_id, user_id);
        updateData();
    };

    const makeAsTeamHead = (team_id) => async (user_id) => {
        await teamService.associateHeadToTeam(team_id, user_id);
        updateData();
    };
